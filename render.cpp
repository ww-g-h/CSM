...
const unsigned int SCR_WINTH = 2560;
...
float lastX = (float)SCR_WINTH / 2.0f;
...
std::vector<float> cascadeSplits = { 10.0f, 20.0f, 50.0f, 250.0f };
int debugLayer = 0;
...
std::mt19937 generator = std::mt19937(decivel());
...
int main()
{
    ...
    GLFWWwindow* window = glfwCreateWindow(SCR_WINTH, SCR_HEIGHT, "CSM", NULL, NULL);
    if(window == NULL)
    {
        st::cout << "Failed to create GLFW window" << std::endl;
        glfwTerminate();
        return -1;
    }
    ...
    Shader shader("Shader/shadow_mapping.vs", "Shader/shadow.frag");
    Shader simpleDepthShader("Shader/shadow_mapping_depth.vs", "Shader/shadow_mapping_depth.frag", "Shader/shadow_mapping_depth.gs");
    Shader debugDepthQuad("Shader/debug_quad.vs", "Shader/debug_quad_depth.frag");
    Shader debugCascadeShader("Shader/debug_cascade.vs", "Shader/debug_cascade.frag");
